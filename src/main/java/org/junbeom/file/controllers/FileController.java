...
import org.junbeom.file.services.FileDeleteService;
import org.junbeom.file.services.FileDownloadService;
import org.junbeom.file.services.FileInfoService;
...
public class FileController implements RestExceptionProcessor {
    ...
    public  JSONData deletes(@PathVariable("gid") String gid, @RequestParam(name="location", required = false)
                             String location) {
       ...
       @GetMapping("/list/{gid}")
       public JSONData getList(@PathVariable("gid") String gid, @RequestParam
               (name="location", required = false)   String location) {
